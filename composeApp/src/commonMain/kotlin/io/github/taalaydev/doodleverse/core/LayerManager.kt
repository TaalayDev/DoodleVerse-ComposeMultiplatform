...
class LayerManager(
    ...
    fun addPathToLayer(frame: FrameModel, layerIndex: Int, path: DrawingPath, bitmap: ImageBitmap): FrameModel {
        if (layerIndex < 0 || layerIndex >= frame.layers.size) return frame

        val newLayers = frame.layers.toMutableList()
        val updatedLayer = newLayers[layerIndex].copy(
            paths = newLayers[layerIndex].paths + path
        )
        newLayers[layerIndex] = updatedLayer
