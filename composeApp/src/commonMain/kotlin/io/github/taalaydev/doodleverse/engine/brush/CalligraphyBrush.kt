...
class CalligraphyBrush(
    private val nibAngleDegrees: Float = 35f,
    private val nibAspect: Float = 0.28f, // height/width ratio
    override val id: ToolId = ToolId("calligraphy"),
    override val name: String = "Calligraphy"
) : ProceduralBrush() {

    override fun startSession(canvas: Canvas, params: BrushParams): StrokeSession =
        Session(canvas, nibAngleDegrees, nibAspect, params)

    private class Session(
        private val canvas: Canvas,
        private val nibAngleDegrees: Float = 35f,
        private val nibAspect: Float = 0.28f,
        params: BrushParams
        ...
        private val stepPx = params.size * 0.35f
        ...
        private fun nibRect(c: Offset, pressure: Float): Rect {
            val width = params.size * pressure.coerceAtLeast(0.15f)
            val height = (width * nibAspect).coerceAtLeast(1f)
            canvas.save()
            canvas.translate(c.x, c.y)
            canvas.rotate(nibAngleDegrees)
            canvas.drawRoundRect(-width * 0.5f, -height * 0.5f, width * 0.5f, height * 0.5f, height * 0.5f, height * 0.5f, paint)
            canvas.restore()
            val pad = max(width, height) * 0.8f
            return Rect(c.x - pad, c.y - pad, c.x + pad, c.y + pad)
        }
        ...
        private fun walkQuadratic(a: Offset, b: Offset, c: Offset, pStart: Float, pEnd: Float): DirtyRect {
            ...
            while (t <= 1f + 1e-4f) {
                ...
                while (acc + remain >= stepPx && remain > 0f) {
                    ...
                    dirty = dirty.union(nibRect(hit, press))
                    prev = hit
                    remain -= need
                    acc = 0f
                }
        ...
        override fun start(event: GestureEvent): DirtyRect {
            ...
            return nibRect(event.position, lastPressure)
        }
        ...
        override fun end(event: GestureEvent): DirtyRect {
            ...
            return moved.union(nibRect(event.position, event.pressure ?: params.pressure))
        }
    }
}
